 ...
 * @date: 2015-2-4 上午
 */
public abstract class BaseFrameworkAdapter<E> extends BaseAdapter {

    private List<E> mList = new ArrayList<>();
    protected Context mContext;
    protected LayoutInflater mInflater;

    public BaseFrameworkAdapter(Context context) {
        mContext = context;
        mInflater = LayoutInflater.from(mContext);
    }

    public BaseFrameworkAdapter(Context context, List<E> list) {
        this(context);
        mList = list;
    }

    public BaseFrameworkAdapter(Activity activity, List<E> list) {
        this(activity);
        mList = list;
    }
